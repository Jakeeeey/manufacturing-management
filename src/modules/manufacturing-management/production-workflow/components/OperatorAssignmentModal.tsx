...
export function OperatorAssignmentModal({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-xs p-4 animate-in fade-in duration-200">
            <div className="bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden p-4 sm:p-6 space-y-4 max-h-[90vh] flex flex-col animate-in zoom-in duration-200">
                {/* Header */}
                <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-3 shrink-0">
                    ...
                    <button
                        ...
                        className="text-muted-foreground hover:text-foreground transition-colors border-none bg-transparent cursor-pointer text-xs p-1 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800"
                    >
                ...
                <div className="flex-1 overflow-y-auto pr-1 space-y-5 min-h-[350px]">
                    ...
                    <div className="space-y-2">
                        ...
                        {assignedUsers.length === 0 ? (
                            <div className="p-4 text-center border border-dashed rounded-xl border-slate-200 dark:border-slate-850 bg-slate-50 dark:bg-slate-950/20 text-muted-foreground text-[10px] font-semibold">
                    ...
                    <div className="space-y-3 pt-2 border-t border-slate-200 dark:border-slate-800/60">
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full bg-slate-955 border border-slate-200 dark:border-slate-800 rounded-xl pl-9 pr-4 py-2 text-xs text-foreground placeholder-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary font-semibold"
                                />
                                {operatorSearchText && (
                                    <button
                        ...
                        <div className="space-y-2">
                            {filteredAvailable.length === 0 ? (
                                <div className="p-6 text-center border border-dashed rounded-xl border-slate-200 dark:border-slate-850 bg-slate-955/20">
                                ...
                                filteredAvailable.map((u, uIdx) => {
                                    ...
                                    const avatarStyle = colors[(u.user_id || 0) % colors.length];
                                    return (
                                        <div key={u.user_id || u.id || uIdx} className="flex items-center justify-between p-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/40 hover:border-slate-750 transition-all">
                                            <div className="flex items-center gap-3">
                                                <div className={`h-8 w-8 rounded-full border flex items-center justify-center font-bold text-xs shrink-0 ${avatarStyle}`}>
                                                    {initials}
                                                </div>
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    <div className="flex items-center gap-2 pt-0.5">
                                                        <div className="w-12 bg-slate-100 dark:bg-slate-850 h-1 rounded-full overflow-hidden">
                                                            <div 
