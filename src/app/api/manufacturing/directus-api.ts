...
export async function fetchJobOrders(): Promise<any[]> {
    try {
        ...
        const [joRes, jopRes, josoRes] = await Promise.all([
...
export async function createJobOrder(joData: any, salesOrderIds?: number[]): Promise<any> {
    try {
        ...
        const headerPayload = {
            ...
            assigned_personnel: joData.assigned_personnel || null
        };
                ...
                try {
                    ...
                    if (joRes.ok && jopRes.ok) {
                        ...
                        if (joData && productsList.length > 0) {
                            ...
                            // 1. Create a placeholder incoming shipment for the manufacturing run (required for shipment_line_items.shipment_id)
                            ...
                            if (shipHeaderRes.ok) {
                                ...
                                for (const p of productsList) {
                                    ...
                                    if (qty > 0) {
                                        ...
                                        // 2. Create a shipment_line_items record (acting as the expiring FIFO lot)
                                        await fetch(`${DIRECTUS_URL}/items/shipment_line_items`, {
                                            ...
                                            body: JSON.stringify({
                                                ...
                                                branch_id: bId,
                                                qa_status: "Passed",
                                                base_unit_cost_php: 0,
                                                allocated_expense_php: 0,
                                                final_landed_unit_cost: 0
                                            })
                                        });
                                        
                                        // 3. Create a product_ledger entry (updating transaction history & stock balances)
                                ...
                                console.error("[Manufacturing Directus API] Failed to create shipment header placeholder:", await shipHeaderRes.text());
                            }
                        }
                    }
                } catch (err) {
