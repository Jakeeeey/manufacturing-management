...
export default async function PickingWorkspacePage(props: { params: Promise<{ consolidatorNo: string }> }) {
    const { consolidatorNo } = await props.params;
    if (!consolidatorNo) {
        redirect("/mm/consolidation/picking");
    }
    ...
    return (
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
            ...
            <main className="min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden bg-background flex flex-col">
                <PickingWorkspaceModule batchNo={consolidatorNo} />
            </main>
        </div>
    );
}
